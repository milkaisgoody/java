...
SELECT COUNT(*) FROM EMP; -- 조건에 맞는 전체 행의 개수
SELECT COUNT(EMP_ID) FROM EMP;
SELECT COUNT(BONUS) FROM EMP; --  컬럼의 값이 NULL이 아닌 컬럼의 개수를 세어줌
...
SELECT AVG(SALARY), 
		...
		ROUND(AVG(SALARY), 1), -- 소수점이하 몇번째까지 보여주는지
...
-- 일치하는 데이터만 조회
-- EMP 테이블과 DEPT테이블은 같은 컬럼을 가지고 있다
-- 테이블의 컬럼이 동일한 이름인 경우 어느 테이블이 가진 컬럼인지 명시적으로 써줘야함
SELECT EMP_NAME, EMP.DEPT_ID, DEPT_TITLE
FROM EMP
-- JOIN DEPT USING(DEPT_ID) -- 컬럼이름이 같을때, 컬럶에 접근할때 테이블 이름을 명시하지 않아도 됨
JOIN DEPT ON EMP.DEPT_ID = DEPT.DEPT_ID; -- 컬럼이름이 다를때

SELECT EMP_NAME, EMP.DEPT_ID, DEPT_TITLE
FROM EMP
LEFT JOIN DEPT USING (DEPT_ID); -- 컬럼이름이 같을때, 컬럼에 접근할때 테이블의 이름을 명시하지 않아도 됨 
...
WHERE DEPT.DEPT_ID = 'D2';

-- 부서코드, 부서명, 부서별 사원수
-- GROUP BY절이 사용된 경우, SELECT 절에 올 수 있는 컬럼은 GROUP BY절에 사용된 컬럼으로 제한
